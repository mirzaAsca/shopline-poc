...
### 3 — Map (source → Bottle)
Per the [§ cheat-sheet](04-bottle-theme-architecture.md#migration-mapping-cheat-sheet-source--bottle):
...
### 5 — Deploy
- `sl theme push --theme ${SL_THEME_ID}` (or `--only` per file) — [02](02-shopline-cli-reference.md).
- Per static page: [`scripts/create-page.sh`](../scripts/create-page.sh) with `template_suffix` + handle + meta — [05](05-pages-and-templates.md).
- Create **menus** and **URL redirects** via Admin API (endpoints to confirm).

### 6 — Validate (the gate)
Isolated Chrome + CDP ([06](06-deploy-publish-validate.md)), at **desktop + mobile** breakpoints:
...
- If the gate passes → publish via **status swap** ([06](06-deploy-publish-validate.md)); record the prior live theme id for rollback.
...
See [09-validation-status](09-validation-status.md) for what is proven vs. still unverified.
