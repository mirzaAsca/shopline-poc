...
**Password protection / "Opening soon":** New/unfinished stores show a password gate (and an "Opening soon" splash) instead of the storefront. Public fetches (and search engines) only see the gate. To view the real storefront you must pass the gate — relevant for validation ([06](deploy-publish-validate.md)) and for any source-scraping of a SHOPLINE origin.
...
- Prefer working against an **unpublished** Bottle theme during a build, then publish when ready ([06](deploy-publish-validate.md)).
