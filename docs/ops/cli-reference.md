...
All commands use the **`sl`** binary from `@shoplineos/cli` (see [01](environment-setup.md)). Run from the theme root unless noted.
...
```
`status: 1` = Published, `0` = Unpublished, `-1` = Delete. This is a **pointer swap** — the previously-live theme becomes `[unpublished]` (intact, reversible). See [06](deploy-publish-validate.md) and [08](../troubleshooting.md#publish-flag-is-a-no-op).
