...
- **Close every task:** tick its `[x]` in the spec, log any decision + why in that spec's Decision log, **append** discoveries to [`/LEARNINGS.md`](../../LEARNINGS.md) **per its header rubric** (journal it; route durable fixes to `docs/` and `validation-status.md` too), commit with detail and push, then **mirror the theme to `shopline-theme`** (`scripts/sync-theme-branch.sh`) for the GitHub-connect deploy.
